...
void Board::flag_cell_toggle(int row, int col){
    //also set win variable to true here if all flags are set in the correct place.
    if(board[row][col]->is_revealed() == false && flagged_cells < 10 && flagged_cells >= 0){
...
void Board::display_board_values(){
    for(int r = 0; r < HEIGHT; r++){
        for(int c = 0; c < WIDTH; c++){
            if(board[r][c]->get_type() == number){
                std::cout << board[r][c]->get_num() << " ";
